...
release=${3;-xenial}sudo apt-get install debootstrapsudo debootstrap --arch=${arch} ${release} chroot ${mirror}
...
apt-get -qq -y install lxqt openbox# Cleaning up the ChRoot environment
rm /sbin/initctl
apt-get -qq clean
rm -rf /tmp/*dpkg-divert --rename --remove /sbin/initctl
exit
EOF
